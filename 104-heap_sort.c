...
/**
 * swap - this sis utility function to swap to integers
 * @a: the integer a
 * @b: the integer b
 **/
...
/**
 * maxHeapify - A main function to heapify a Max Heap. The function
...
void maxHeapify(int *array, size_t size, int idx, size_t n)
{
	...
	/* this See if left child of root exists and is greater than root*/
	if (left < (int)n && array[left] > array[largest])
		largest = left;

	/**
	 * this See if right child of root exists and is greater than
     *the largest so far
	 */
	if (right < (int)n && array[right] > array[largest])
		largest = right;

	/* this Change root, if needed*/
	if (largest != idx)
	{
...
/**
 * heap_sort -  A main function to sort an array of given size
...
void heap_sort(int *array, size_t size)
{
	int i;
	/**
	 * this start from bottommost and rightmost internal mode and heapify all
